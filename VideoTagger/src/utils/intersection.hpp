...
{
	constexpr float cross_product(const ImVec2& p0, const ImVec2& p1, const ImVec2& p2)
	{
		return (p1.x - p0.x) * (p2.y - p0.y) - (p1.y - p0.y) * (p2.x - p0.x);
	}

	bool is_convex_polygon(const std::vector<ImVec2>& points)
	{
	...
	bool is_concave_polygon(const std::vector<ImVec2>& points)
	{
	...
	bool is_in_circle(const ImVec2& pos, const ImVec2& circle_pos, float radius)
	{
		return length(pos - circle_pos) <= radius;
	}

	bool is_in_rect(const ImVec2& pos, const ImRect& rect)
	{
		return (pos.x >= rect.Min.x and pos.x <= rect.Max.x) and (pos.y >= rect.Min.y and pos.y <= rect.Max.y);
	}

	bool is_on_line(const ImVec2& pos, const ImVec2& p1, const ImVec2& p2)
	{
	...
	bool is_in_polygon(const ImVec2& pos, const std::vector<ImVec2>& points)
	{
