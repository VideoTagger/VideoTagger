...
namespace vt
{
	void points_tool::render_overlay(video_id_t video_id, ImVec2 pos, ImVec2 size, ImVec2 tex_size)
	{
		shape_tool<points_shape>::render_overlay(video_id, pos, size, tex_size);
		...
		if (shape_data.has_value())
		{
			...
			shape_data->render(utils::vec2<uint32_t>({ (uint32_t)tex_size.x, (uint32_t)tex_size.y }), pos, pos + size, tag.fill_color() | 0xFF000000, tag.outline_color(), std::nullopt);
