...
namespace vt::ui
{
	wand_sam2_extension::wand_sam2_extension(const std::string& name) : impl::wand_tool_extension{ name }, mode_{ wand_sam2_mode::rectangle }, is_fg_point_{ true }
	{
	...
	void wand_sam2_extension::handle_point_selection(video_id_t video_id, ImRect draw_rect, const utils::vec2<int>& tex_size)
	{
		...
		if (is_hovered and is_mouse_clicked)
		{
			...
			if (is_ctrl_held)
			{
				for (auto& current_points_shape : { &foreground_points_, &background_points_ })
				{
	...
	void wand_sam2_extension::generate_mask(video_id_t video_id, const utils::vec2<int>& tex_size, const std::optional<utils::vec4<float>>& rect)
	{
		...
		switch (mode_)
		{
			...
			{
				if (foreground_points_.points.empty() and background_points_.points.empty())
				{
					auto mask_data = data();
					if (mask_data != nullptr)
					{
				...
				ctx_.tasks.run([this, rect = rect, &vid_data]()
				{
					set_busy(true);
	...
	void wand_sam2_extension::draw_remove_point_preview(const ImVec2& center, float brush_size)
	{
	...
	void wand_sam2_extension::render_overlay(video_id_t video_id, ImVec2 pos, ImVec2 size, ImVec2 tex_size)
	{
		...
		switch (mode_)
		{
			...
			case wand_sam2_mode::points:
			{
				handle_point_selection(video_id, ImRect(pos, pos + size), { static_cast<int>(tex_size.x), static_cast<int>(tex_size.y) });
			}
			break;
