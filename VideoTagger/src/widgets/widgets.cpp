...
namespace vt::widgets
{
	...
	void draw_timeline_widget_sample(video& video, tag_storage& tags, std::optional<selected_timestamp_data>& selected_timestamp, std::optional<moving_timestamp_data>& moving_timestamp, bool& dirty_flag, uint32_t id)
	{
		...
		if (ImGui::Begin(title.c_str()))
		{
			...
			video_timeline(test_timeline, current_time, selected_timestamp, moving_timestamp, dirty_flag);
			ImGui::PopStyleVar();
