...
namespace vt
{
	...
	sam2_encoder_result sam2_image_encoder::encode(const image<image_pixel_format::rgb8>& img)
	{
		...
		for (size_t i = 0; i < output_tensors.size(); ++i)
		{
			...
			switch (i)
			{
				case 0: result.high_res_feats_0_value = std::move(val); break;
				case 1: result.high_res_feats_1_value = std::move(val); break;
				case 2: result.image_embed_value = std::move(val); break;
			}
		}
		return result;
	}
