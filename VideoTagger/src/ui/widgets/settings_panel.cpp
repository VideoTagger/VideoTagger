...
namespace vt::ui
{
    ...
    settings_panel& settings_panel::add_button_cond(const std::string& label, const std::string& description, const std::string& button_label, const std::function<void()>& on_click, const std::function<bool()>& condition)
    {
		add<settings_expander>(label, description, [label, button_label, on_click, condition](float height)
		{
			...
			ImGui::BeginDisabled(!condition_result);
			bool result = ui::rounded_button(button_label);
			ImGui::EndDisabled();
			ui::end_bigger_frames();
