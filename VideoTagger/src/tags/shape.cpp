...
namespace vt
{
	...
	static void draw_keyframes(const std::string& shape_name, std::map<timestamp, std::vector<type>>& map, bool is_modifiable, bool& dirty_flag, timestamp ts, const std::function<void(typename std::map<timestamp, std::vector<type>>::mapped_type::value_type&, size_t i)>& draw_shape, const std::function<void(void)>& on_vec_modified, const std::function<void(timestamp)>& on_seek)
	{
		const auto& style = ImGui::GetStyle();
		...
		if (ImGui::BeginTable(fmt::format("##Shape{}Keyframes", shape_name).c_str(), 1))
		{
			ImVec2 separator_pos;
			ImGui::TableNextColumn();
			if (is_modifiable and widgets::icon_button(icons::add))
			{
				map[ts].push_back({});
				on_vec_modified();
			}
			...
			for (auto it = map.begin(); it != map.end();)
			{
				...
				if (is_collapsible_open)
				{
					...
					//TODO: Set proper min_ts and max_ts
					if (widgets::timestamp_control("Keyframe", ts, 0, 0, nullptr, nullptr))
					{
	...
	void shape::draw_data(const utils::vec2<uint32_t>& max_size, utils::vec2<uint32_t>*& gizmo_target, timestamp ts, bool is_timestamp, bool is_modifiable, bool& dirty_flag, const std::function<void(timestamp)>& on_seek)
	{
		const auto& style = ImGui::GetStyle();
		...
		switch (type_)
		{
			...
			case shape::type::circle:
			{
				...
				//if (ImGui::BeginTable("##ShapeCircleKeyframes", 1))
				//{
				...
				//		bool is_current_keyframe = keyframe == ts;
				//		ImGui::PushID((void*)keyframe.total_milliseconds.count());
				//		if (widgets::begin_collapsible("##ShapeKeyframe", utils::time::time_to_string(keyframe.total_milliseconds.count()), 0, is_current_keyframe ? icons::keyframe_current : icons::keyframe, is_current_keyframe ? std::optional<ImVec4>{ ImGui::ColorConvertU32ToFloat4(selected_color) } : std::nullopt))
				//		{
				//			if (ImGui::BeginTable("##ShapeCircleRegions", 1))
				//			{
				//				for (size_t i = 0; i < regions.size(); ++i)
				//				{
				//					auto& v = regions[i];
				...
				//				was_deleted = true;
				//			}
				//			if (ImGui::MenuItem(fmt::format("{} Go To", icons::delete_).c_str()))
				//			{
				//				on_seek(keyframe);
				//			}
				//			ImGui::EndPopup();
				...
				is_modifiable &= (is_timestamp and map.empty()) or !is_timestamp;
				draw_keyframes("Circle", map, is_modifiable, dirty_flag, ts, [&gizmo_target, &max_size, &style](circle& v, size_t i)
				{
					auto cpos = ImGui::GetCursorPos();
					bool selected = gizmo_target == &v.pos;
					if (ImGui::Selectable("##CircleSelectable", selected, ImGuiSelectableFlags_AllowItemOverlap | ImGuiSelectableFlags_AllowOverlap | ImGuiSelectableFlags_SpanAllColumns, ImVec2{ 0.f, ImGui::GetTextLineHeightWithSpacing() + 2 * style.FramePadding.y + style.ItemSpacing.y } *3.f))
